...
impl SigGenCallback {
    ...
    fn dump_mir(
        ...
        file.write_all(serde_json::to_string(&self.all_mirs)
...
impl Callbacks for SigGenCallback {
    fn after_analysis<'tcx>(
        ...
        queries.global_ctxt().unwrap().peek_mut().enter(|tcx| {
            // Only dump mir when generating the final bin crate.
            // To avoid to much duplication.
            let target_crate = env::var("RUSTC_MIR_TARGET_CRATE").unwrap_or("".to_string());
            // It seems that current/local crate always have number zero
            let local_crate_num = CrateNum::from_u32(0);
            let local_crate_name = tcx.crate_name(local_crate_num).to_ident_string();
            if local_crate_name != target_crate {
                return;
            }

            // Prepare dump dir.
            let dump_dir = PathBuf::from(
                env::var("RUSTC_MIR_DUMP_DIR").unwrap_or("mir_info".to_string())
            );
            fs::create_dir_all(&dump_dir).unwrap();

            let dump_crate_list = env::var("RUSTC_MIR_DUMP_CRATES")
                                    .unwrap_or("".to_string())
                                    .split(":")
                                    .map(String::from)
                                    .collect::<Vec<_>>();
    
            // rustc_middle::ty::context::TyCtxt.crates(): 
            //   fetching all    *FOREIGN*    CrateNum instances
            // It seems that can not dump the info of local crate...
            for crate_num in tcx.crates(()) {
                let crate_name = tcx.crate_name(*crate_num).to_ident_string();
                if !dump_crate_list.iter().any(|crt| crate_name.eq(crt)) {
                    continue;
                }
                let crate_store = tcx
                    .cstore_untracked()
                ...
                let num_def_ids = crate_store.num_def_ids_untracked(*crate_num);
                for def_index in 0..num_def_ids {
                    let def_id = DefId {
                        krate: *crate_num,
                        index: DefIndex::from_usize(def_index),
                    };

                    if tcx.is_mir_available(def_id) {
                        let mir = tcx.optimized_mir(def_id);
                        self.record_mir(mir, def_id, tcx);
                    }
                }
                self.dump_mir(&dump_dir, crate_name);
            }
